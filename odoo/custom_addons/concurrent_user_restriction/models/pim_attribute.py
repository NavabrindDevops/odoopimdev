...
class PIMAttributeType(models.Model):
    ...
    def can_edit_session(self):
        if not self.env.user.has_group('pim_ext.group_admin_user') or not self.id:
            return 2
    ...
    def clear_edit_session(self):
        ...
        if self.env.user.has_group('pim_ext.group_admin_user'):
            ...
            session_details = self.env['session.info'].search(
                [('session_id', '=', self_session), ('pim_attribute_id', '=', self.id)])
    ...
    def get_editing_session(self, session):
        sessions = self.env['session.info'].search([('pim_attribute_id', '=', self.id), ('active_state', '=', True)],
    ...
    @api.onchange('name', 'display_type', 'code', 'attribute_group', 'unique_value', 'value_per_channel',
                  'value_per_locale',
                  'usable_in_grid', 'locale_specific', 'mandatory', 'is_required_in_clone', 'is_completeness')
    def edit_mode(self):
        if self.env.user.has_group('pim_ext.group_admin_user'):
            if self.editing:

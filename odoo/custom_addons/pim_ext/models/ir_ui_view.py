...
class IrView(models.Model):
    ...
    def _create_new_view(self):
        print('-----------------',self)
        records = self.search([('user_view_parent_id', '=', self._origin.id), ('type', '=', 'tree')])
    ...
    def save_custom_list(self, description, view_id=None, res_model=None, optional_fields=None, filter_name=None):
        ...
        main_view = self.env['ir.ui.view'].sudo().search(
            [('id', '=', view_id), ('model', '=', res_model), ('type', '=', 'tree')]
        )
        list_view_count = self.env['ir.ui.view'].sudo().search_count(
            [('model', '=', self._origin.model), ('type', '=', 'tree'),
    ...
    def get_custom_view_list(self):
        list_view = self.env['ir.ui.view'].sudo().search([('shared_user_id','=',self.env.user.id),('model', '=', self._origin.model), ('type', '=', 'tree'),
                                                   ('active', '!=',None)])

        # list_view = list_view.filtered(lambda r : self.env.user.id in r.share_user_ids.ids or self.env.user.id == r.create_uid.id)
        default_view = self.env['ir.ui.view'].sudo().search([('model', '=', self._origin.model), ('type', '=', 'tree'),
        ...
        selected_view = self.env['ir.ui.view'].sudo().search([('model', '=', self._origin.model), ('type', '=', 'tree'),
